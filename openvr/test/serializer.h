...
public:
  ...
  inline Serializer &operator>>(std::string &t) {
    ...
    if (in.m_read % 8 != 0) {
      in.m_read += 8 - (in.m_read % 8);
    }
    
    // getOut() << "reader string 1 " << (in.m_read - in.m_data) << std::endl;
    if ((in.m_data + in.m_read + sizeof(size_t)) > (in.m_data + m_size)) {
    ...
    size_t size = *((size_t *)(in.m_data + in.m_read));
    // getOut() << "read size 1 " << id << " " << size << " " << (void *)in.m_read << std::endl;
    in.m_read += sizeof(size_t);
    
    if ((in.m_data + in.m_read + size) > (in.m_data + m_size)) {
    ...
    t = std::string((char *)(in.m_data + in.m_read), size);
    in.m_read += size;
  ...
  inline Serializer &operator>>(managed_binary<T> &t) {
    ...
    if ((in.m_data + in.m_read + size*sizeof(T)) > (in.m_data + m_size)) {
    ...
    t = managed_binary<T>((T *)(in.m_data + in.m_read), size);
  ...
  inline Serializer &operator<<(const T &t) {
    ...
    static_assert(std::is_trivially_copyable<T>::value, "Must be trivially copyable");
    if ((out.m_data + out.m_write + sizeof(T)) > (out.m_data + m_size)) {
    ...
    memcpy(out.m_data + out.m_write, (void *)&t, sizeof(T));
  ...
  inline Serializer &operator<<(const std::string &t) {
    ...
    if (out.m_write % 8 != 0) {
      ...
      out.m_write += 8 - (out.m_write % 8); 
    }
    
    // getOut() << "writer string 1 " << (out.m_write - out.m_data) << std::endl;
    if ((out.m_data + out.m_write + sizeof(size_t)) > (out.m_data + m_size)) {
    ...
    *((size_t *)(out.m_data + out.m_write)) = t.size();
    // getOut() << "write size 1 " << id << " " << t.size() << " " << (void *)out.m_write << std::endl;
    out.m_write += sizeof(size_t);
    
    if ((out.m_data + out.m_write + t.size()) > (out.m_data + m_size)) {
    ...
    memcpy(out.m_data + out.m_write, (void *)t.data(), t.size());
    out.m_write += t.size();
  ...
  inline Serializer &operator<<(const binary<T> &t) {
    ...
    *((size_t *)(out.m_data + out.m_write)) = t.size();
    ...
    if ((out.m_data + out.m_write + t.size()*sizeof(T)) > (out.m_data + m_size)) {
      // getOut() << "reset writer binary 2 " << (out.m_write - out.m_data) << " " << t.size() << " " << sizeof(T) << std::endl;
      out.m_write = 0;
    }

    memcpy(out.m_data + out.m_write, (void *)t.data(), t.size() * sizeof(T));
